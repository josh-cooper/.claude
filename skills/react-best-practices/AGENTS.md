...
1. [Eliminating Waterfalls](#1-eliminating-waterfalls) — **CRITICAL**
   - 1.0 [Choose the Right Data Fetching Pattern](#10-choose-the-right-data-fetching-pattern)
   - 1.1 [Defer Await Until Needed](#11-defer-await-until-needed)
   - 1.2 [Dependency-Based Parallelization](#12-dependency-based-parallelization)
   - 1.3 [Parallelize Operations in Route Handlers](#13-parallelize-operations-in-route-handlers)
...
### 1.0 Choose the Right Data Fetching Pattern

**Impact: CRITICAL (correct architecture prevents over-engineering and improves performance)**
...
### 1.3 Parallelize Operations in Route Handlers

**Impact: CRITICAL (2-10× improvement)**

> **When to use Route Handlers:** Only create Route Handlers (API routes) when you need: endpoints for non-React clients, polling/WebSocket connections, explicit cache headers, or shared endpoints across multiple consumers. For simple mutations, use Server Actions instead. See section 1.0 for the full decision guide.
...
> **When to use Server Actions:** Use Server Actions for mutations triggered by user interactions (form submissions, button clicks). They're the preferred pattern for simple mutations - don't create Route Handlers (API routes) for operations that only your React app consumes. See section 1.0 for the full decision guide.

Server Actions (functions with `"use server"`) are exposed as public endpoints, just like API routes. Always verify authentication and authorization **inside** each Server Action—do not rely solely on middleware, layout guards, or page-level checks, as Server Actions can be invoked directly.

Server Actions should be treated with the same security considerations as public-facing API endpoints. Always verify if the user is allowed to perform a mutation.
...
```tsx
// Module-level state for shared subscription
...
function useKeyboardShortcut(key: string, callback: () => void) {
  useEffect(() => {
    ...
    // Set up shared listener on first subscription
    if (listenerCount === 0) {
    ...
    return () => {
      ...
      // Clean up shared listener on last unsubscription
      listenerCount--
      if (listenerCount === 0) {
...
> **When to use:** Only reach for TanStack Query when you genuinely need client-side features like automatic retries, background refetching, or optimistic updates. For most data needs, prefer Server Components + URL params + Server Actions. See section 1.0 for the full decision guide.
